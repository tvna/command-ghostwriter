...
- クラス名はPascalCase[例: MyClass]
- 関数名と変数名はsnake_case[例: my_function]
- 定数はALL_CAPS[例: MAX_SIZE]
- プライベートメンバーには先頭にアンダースコア[例: _private_method]
...
- このルールの適応は、コード回答の直前とします。
- 保守性に問題が出てしまうため、必ず実施することとします。

### フォーマットの徹底

- `pyproject.toml`における、`[tool.ruff]`の設定に従ってフォーマットを実施すること。
- 構文チェックのルールを追加する場合、`pyproject.toml`に追加すること。

### 不要なコードの削除

- 使用されていない変数や関数、コメントアウトされたコードの削除を徹底的に実施する。
...
- 日本語でコメント生成するにあたり、コメント内で全角の丸括弧は利用せず、半角ブラケット`[]`の利用を徹底すること。
- 日本語で生成されたコメントに含まれる全角記号は、全て半角記号に置換すること。
