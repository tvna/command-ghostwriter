...
set network ike gateway IKE-GW-TO-REMOTE local-address interface ethernet1/1
...
事前共有鍵の実際の値は、コンソール入力時のみ台帳(鍵管理システム)から参照し、`set`コマンドの履歴や構成バックアップに平文で残さないようにします。`local-address interface`の`ethernet1/1`は例示であり、実運用では自拠点WAN側の実インターフェース名に置き換えてください。

### 3. 経路とSecurity Policyを設定する

トンネルインターフェースを仮想ルータに割り当てたうえで対向拠点LAN宛の経路を追加し、あわせてLAN側ゾーン(`trust`、既存のLANインターフェースが所属している前提)と`VPN-TRUST`ゾーンの間の通信をSecurity Policyで明示的に許可します。PAN-OSはゾーン間通信を既定で拒否(interzone-default deny)するため、IKE/IPsec SAが確立してもこのルールがなければLAN間の通信は許可されません。
...
set rulebase security rules ALLOW-TO-REMOTE from trust to VPN-TRUST source any destination any application any service application-default action allow
set rulebase security rules ALLOW-FROM-REMOTE from VPN-TRUST to trust source any destination any application any service application-default action allow
...
- Security Policy(`ALLOW-TO-REMOTE`/`ALLOW-FROM-REMOTE`)を投入し忘れると、IKE/IPsec SAが確立していてもPAN-OSの既定拒否(interzone-default deny)によりLAN間トラフィックがドロップされる。トンネル確立とポリシー許可は別レイヤーであることを踏まえて両方を確認する。
```
...
```bash
show crypto-local ipsec tag s2s-branch-to-hq
...
- `show crypto-local ipsec tag s2s-branch-to-hq`で、IKEv2ポリシー10・transform-set(`ikev2-aes256-sha256`)・PFSグループ14・SAライフタイム3600秒が意図通り反映され、トンネル状態(IKE/IPSEC)が`UP`であること
