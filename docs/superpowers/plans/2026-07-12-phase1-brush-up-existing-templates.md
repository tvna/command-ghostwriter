...
- **VLAN**: 1台のスイッチを論理的に複数のネットワークに分割する仕組み
- **トランクポート**: 複数 VLAN のトラフィックを同時に通す、スイッチ間の接続に使うポート設定
- **DHCP リレー**: 別セグメントにいる DHCP サーバへクライアントの要求を中継する仕組み・機器
```

> 実装時の code-quality レビューで、手順本文が実際に使う VLAN / トランクポート / DHCP リレーが当初案の6語に含まれておらず未解説だったため、上記3語を追記して9語構成に修正した（`docs/superpowers/plans/` は実装後の実態を反映するようここも更新済み）。
