...
自己レビューで、クリティカルパスは `guards(74s) -> unit(36s) -> E2E(560s)` であり、
E2E を `small-unit-test` から切り離さない限り sub-1 単独では壁時間が改善しないことが判明した。
方針確認の結果:

- **E2E は `small-unit-test` ゲートを維持する**(安価な単体テストで高コストE2Eを fail-fast
  する現行設計は billable バランス上むしろ良い)。
- したがって**壁時間の本命である sub-2(E2Eマトリクス再設計)を先行実装**する。
- sub-1(DAG再編)は E2E のゲートを動かさない範囲(zap/build/analysis の前倒し)に縮小し、
  後続で実施するか、効果が小さければ見送る。
...
- sub-2(先行・本命): E2E を ubuntu(1x・高速)中心に再編し、クリティカルパスの長尺
  (E2E windows 560s)を短縮 + macOS 10x 分の課金を大幅減(概算 -80% の重み付き課金)。
- sub-1(縮小・後続): E2E ゲートは維持。zap/build/analysis を guards 依存に付け替え、
  これら非クリティカルジョブを約36秒前倒し(壁時間への寄与は限定的)。
- 合計: 緑時の壁時間短縮の大半は sub-2 に依存(数値は実CIで確定)。
