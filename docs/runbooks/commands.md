...
# macosにおけるcompletion
uv generate-shell-completion zsh > $(brew --prefix)/share/zsh-completions/_uv
```
...
```bash
# 特定のブラウザでテストを実行
uv run pytest -vv -n auto --browser chromium -m "benchmark" --benchmark-disable
uv run pytest -vv -n auto --browser firefox -m "benchmark" --benchmark-disable
uv run pytest -vv -n auto --browser webkit -m "benchmark" --benchmark-disable
```
