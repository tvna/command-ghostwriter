...
import type { ComputeError, Format } from '../lib/engine';
...
export function Editor({ initial, onBack }: EditorProps) {
  ...
  const r = React.useMemo(() => compute(dataText, format, tplText), [dataText, format, tplText]);
  const blocked = !r.ok;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', minHeight: 620, minWidth: 1024, background: 'var(--cg-bg)', fontFamily: 'var(--font-sans)', color: 'var(--cg-text)' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: '0 18px', height: 56, borderBottom: '1px solid var(--cg-border)', flexShrink: 0 }}>
        ...
        <Button variant="ghost" size="sm" icon={<Icon name="settings" size={15} />} onClick={() => setSettings(true)}>詳細設定</Button>
      </header>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 7px 1fr', gridTemplateRows: 'minmax(0, 1fr)', minHeight: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                {r.suggest && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 9 }}>
                    <button onClick={() => setFormat(r.suggest!)} style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--text-xs)', fontWeight: 600, color: '#fff', background: 'var(--cg-red)', border: 'none', borderRadius: 'var(--radius-sm)', padding: '5px 11px', cursor: 'pointer' }}>
                      {r.suggest.toUpperCase()} として読み込む
                    </button>
                    <button onClick={() => setFormat(r.suggest!)} style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--text-xs)', color: 'var(--cg-text-muted)', background: 'transparent', border: '1px solid var(--cg-border-strong)', borderRadius: 'var(--radius-sm)', padding: '5px 11px', cursor: 'pointer' }}>
                      自動判定で修正
                    </button>
                  </div>
                )}
        ...
        <section style={{ display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0, overflow: 'hidden' }}>
          <PaneHeader>
            ...
            {rightMode !== 'debug' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Selectbox value={enc} options={['UTF-8', 'Shift_JIS']} onChange={setEnc} style={{ width: 152 }} />
                <Button variant="secondary" size="sm" disabled={blocked} icon={<Icon name="copy" size={14} />} onClick={() => fire('コピーしました')}>コピー</Button>
                <Button variant="primary" size="sm" disabled={blocked} icon={<Icon name="download" size={14} />} onClick={() => fire('ダウンロードを開始')}>保存</Button>
              </div>
            )}
          </PaneHeader>
...
function BlockedOutput({ format, error, suggest, onFix }: { format: Format; error: ComputeError | null; suggest: Format | null; onFix: () => void }) {
