...
const { Button, FileUploader, Badge } = window.CommandGhostwriterDesignSystem_0d5f31;
...
function rng(n) { const x = Math.sin((n + CG_SEED) * 99.13) * 43758.5453; return x - Math.floor(x); }
...
function ConceptCard({ icon, title, sub, note, outcome }) {
  return (
    <div style={{
      width: 184, background: 'var(--cg-bg-secondary)',
      border: `1px solid ${outcome ? 'var(--cg-error-border)' : 'var(--cg-border)'}`,
      borderRadius: 'var(--radius-md)', padding: '16px 16px 14px',
...
function Op({ children, accent }) {
  return (
    <div style={{ display: 'grid', placeItems: 'center', width: 34, flexShrink: 0,
                  fontSize: 22, color: accent ? 'var(--cg-red)' : 'var(--cg-text-faint)', fontWeight: 700 }}>
      {children}
    </div>
  );
}

function EmptyState({ onStart, onLibrary }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--cg-bg)',
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: '0 18px',
        ...
        <div style={{ flex: 1 }} />
        <a href="#" onClick={(e) => e.preventDefault()} style={{ color: 'var(--cg-text-muted)',
            textDecoration: 'none', fontSize: 'var(--text-sm)' }}>使い方</a>
      </header>

      {/* ===== haunting atmosphere (behind content) ===== */}
      <div aria-hidden="true" style={{ position: 'absolute', top: 56, left: 0, right: 0, bottom: 0,
        ...
        <div className="cg-glow" style={{ position: 'absolute', top: '4%', left: '50%', width: 460, height: 460,
          marginLeft: -230, borderRadius: '50%', filter: 'blur(50px)',
          background: 'radial-gradient(circle, rgba(174,199,214,.5), rgba(174,199,214,0) 70%)' }} />
        {/* drifting infra glyphs */}
        {CG_GLYPHS.map((g, i) => (
          <span key={'g' + i} className={g.anim} style={{ position: 'absolute', top: g.top, left: g.left,
            opacity: g.op, animationDelay: g.delay }}>
            <window.Icon name={g.name} size={g.size} color="var(--cg-ghost-outline)" />
          </span>
        ))}
        {/* whispered command fragments */}
        {CG_WHISPERS.map((w, i) => (
          <span key={'w' + i} className="cg-fade" style={{ position: 'absolute', top: w.top, left: w.left,
            opacity: 0.1, fontFamily: 'var(--font-mono)', fontSize: w.size, color: 'var(--cg-ghost-outline)',
            whiteSpace: 'nowrap', letterSpacing: '.05em', animationDelay: w.delay }}>{w.text}</span>
        ))}
        {/* dust motes */}
        {CG_MOTES.map((m, i) => (
          <span key={'m' + i} className="cg-ghost" style={{ position: 'absolute', bottom: -10, left: m.left,
            width: m.size, height: m.size, borderRadius: '50%', background: 'var(--cg-ghost-outline)',
            opacity: 0, '--cg-d': m.dur, '--cg-delay': m.delay, '--cg-o': m.op }} />
        ))}
        {/* low mist — layered, creeping */}
        <div className="cg-mist" style={{ position: 'absolute', bottom: -100, left: '50%', width: 1240, height: 320,
          marginLeft: -620, borderRadius: '50%', filter: 'blur(58px)',
          background: 'radial-gradient(circle, rgba(174,199,214,.32), rgba(174,199,214,0) 70%)' }} />
        <div className="cg-creep" style={{ position: 'absolute', bottom: 0, left: -120, right: -120, height: 180,
          filter: 'blur(34px)',
          background: 'linear-gradient(180deg, rgba(174,199,214,0), rgba(174,199,214,.18))' }} />
        <div className="cg-mist" style={{ position: 'absolute', top: '36%', left: '50%', width: 1040, height: 200,
          marginLeft: -520, borderRadius: '50%', filter: 'blur(50px)', animationDelay: '-3s',
        ...
        <div style={{ width: '100%', maxWidth: 880 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 30 }}>
            <div style={{ position: 'relative', width: 76, height: 76, margin: '0 auto 18px' }}>
              <div className="cg-glow" style={{ position: 'absolute', inset: -26, borderRadius: '50%',
                filter: 'blur(22px)', background: 'radial-gradient(circle, rgba(174,199,214,.6), rgba(174,199,214,0) 70%)' }} />
              <img src="../assets/brand/logo-mark.svg" alt="" className="cg-bob"
            ...
            <div className="cg-flicker" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)',
              letterSpacing: '.18em', color: 'var(--cg-ghost-outline)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginBottom: 34, flexWrap: 'wrap' }}>
            <ConceptCard icon={<window.Icon name="config-file" size={26} />} title="設定定義ファイル" sub="TOML · YAML · CSV" note="「値」を定義" />
            <Op>＋</Op>
            <ConceptCard icon={<window.Icon name="template-file" size={26} />} title="Jinjaテンプレート" sub=".j2 · .jinja2" note="「雛形」を記述" />
            <Op accent>→</Op>
            <ConceptCard icon={<window.Icon name="terminal" size={26} color="var(--cg-red)" />} title="実行可能なコマンド" sub="CLI · Markdown" note="そのまま実行" outcome />
          </div>

          {/* start card */}
          <div style={{ background: 'var(--cg-bg-secondary)', border: '1px solid var(--cg-border)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <Button variant="primary" size="lg" icon={<window.Icon name="generate" size={17} />} onClick={() => onStart('sample')}>サンプルで試す</Button>
            </div>
            ...
            <div style={{ marginTop: 18, textAlign: 'center' }}>
              <a href="#" onClick={(e) => { e.preventDefault(); onLibrary && onLibrary(); }}
                 style={{ color: 'var(--cg-info)', textDecoration: 'none', fontSize: 'var(--text-sm)', fontWeight: 600 }}>
                保存済みテンプレートライブラリから選ぶ →
              </a>
            </div>
          </div>
