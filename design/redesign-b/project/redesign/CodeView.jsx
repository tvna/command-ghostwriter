...
function cvTokens(line, lang) {
  ...
  while ((m = re.exec(line))) {
    ...
    let gi = 1; for (; gi < m.length; gi++) { if (m[gi] != null) break; }
...
function CodeView({ code, lang, errorLine, onChange, readOnly }) {
  ...
  const PAD = '12px 16px';
  return (
    <div style={{ display: 'flex', position: 'relative', fontFamily: 'var(--font-mono)', fontSize: 13,
                  lineHeight: '20px', background: 'var(--cg-bg-code)', height: '100%', overflow: 'hidden' }}>
      {/* gutter */}
      <div ref={gutRef} style={{ flexShrink: 0, textAlign: 'right', padding: '12px 10px 12px 14px',
                    color: 'var(--cg-text-faint)', userSelect: 'none', overflow: 'hidden',
                    background: 'rgba(255,255,255,.015)', borderRight: '1px solid var(--cg-border)' }}>
        {lines.map((_, i) => (
          <div key={i} style={{ color: i + 1 === errorLine ? 'var(--cg-red)' : undefined }}>{i + 1}</div>
        ))}
      </div>
      ...
      <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
        {/* highlighted layer */}
        <div ref={hlRef} style={{ position: 'absolute', inset: 0, padding: PAD, whiteSpace: 'pre',
                      overflow: editable ? 'hidden' : 'auto', color: 'var(--cg-text)',
                      pointerEvents: editable ? 'none' : 'auto' }}>
          {lines.map((line, i) => (
            <div key={i} style={{ minHeight: 20,
                  background: i + 1 === errorLine ? 'var(--cg-error-bg)' : undefined }}>
              {cvTokens(line, lang).map((tok, j) => (
                <span key={j} style={tok.c ? { color: CV_COLORS[tok.c] } : undefined}>{tok.t}</span>
              ))}
              {line === '' && <span>{'\u200b'}</span>}
            </div>
          ))}
        ...
        {editable && (
          <textarea
            ...
            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', margin: 0,
              padding: PAD, border: 'none', outline: 'none', resize: 'none', background: 'transparent',
              fontFamily: 'inherit', fontSize: 'inherit', lineHeight: 'inherit', whiteSpace: 'pre',
              overflow: 'auto', color: 'transparent', caretColor: 'var(--cg-text)', tabSize: 2 }}
          />
        )}
      </div>
    </div>
