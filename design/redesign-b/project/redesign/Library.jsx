...
const { Badge } = window.CommandGhostwriterDesignSystem_0d5f31;
...
const FMT_TONE = { toml: 'brand', yaml: 'info', csv: 'warning' };
const OUT_LABEL = { cli: 'CLI', config: 'config', markdown: 'Markdown' };

function CatIcon({ name, size, color }) { return <window.Icon name={name} size={size} color={color} />; }
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 'var(--radius-md)', padding: 16, gap: 10, minHeight: 158,
        transition: 'border-color var(--dur-base), transform var(--dur-base)',
        transform: hover ? 'translateY(-2px)' : 'none', fontFamily: 'var(--font-sans)',
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ width: 34, height: 34, borderRadius: 'var(--radius-sm)', background: 'var(--cg-bg)',
                       border: '1px solid var(--cg-border)', display: 'grid', placeItems: 'center' }}>
          <CatIcon name={CATS.find((c) => c.id === tpl.category).icon} size={18} color="var(--cg-red)" />
        </span>
        {tpl.live && <Badge tone="success">ライブ</Badge>}
      </div>
      <div style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--cg-text)', lineHeight: 1.35 }}>
        {tpl.name}
      </div>
      <div style={{ fontSize: 'var(--text-xs)', color: 'var(--cg-text-muted)', lineHeight: 1.55, flex: 1 }}>
        {tpl.desc}
      </div>
...
function Library({ onOpen, onClose }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--cg-bg)',
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: '0 18px',
        ...
        <div style={{ flex: 1 }} />
        {onClose && (
          <button onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer',
            background: 'transparent', border: '1px solid var(--cg-border)', borderRadius: 'var(--radius-md)',
            color: 'var(--cg-text-muted)', fontFamily: 'var(--font-sans)', fontSize: 'var(--text-sm)', padding: '6px 12px' }}>
            ← 戻る
          </button>
        )}
      </header>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        {/* left category rail */}
        <nav style={{ width: 220, flexShrink: 0, borderRight: '1px solid var(--cg-border)',
                      background: 'var(--cg-bg-secondary)', padding: 'var(--space-4)' }}>
          <div style={{ fontSize: 'var(--text-2xs)', textTransform: 'uppercase', letterSpacing: '.06em',
                        color: 'var(--cg-text-faint)', fontWeight: 700, padding: '4px 8px 10px' }}>カテゴリ</div>
          {CATS.map((c) => {
            const on = c.id === cat;
            return (
              <button key={c.id} onClick={() => setCat(c.id)} style={{
                width: '100%', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer',
                background: on ? 'rgba(255,75,75,.1)' : 'transparent', border: 'none',
                borderRadius: 'var(--radius-sm)', padding: '8px 9px', marginBottom: 2,
                color: on ? 'var(--cg-red-tint)' : 'var(--cg-text)', fontFamily: 'var(--font-sans)',
                fontSize: 'var(--text-sm)', fontWeight: on ? 600 : 400, textAlign: 'left',
              }}>
        ...
        <main style={{ flex: 1, minWidth: 0, overflow: 'auto', padding: 'var(--space-8)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(244px, 1fr))', gap: 14 }}>
            ...
            <button onClick={() => onOpen(null)} style={{
              ...
              justifyContent: 'center', gap: 10, minHeight: 158, background: 'transparent',
              border: '1px dashed var(--cg-border-strong)', borderRadius: 'var(--radius-md)',
              color: 'var(--cg-text-muted)', fontFamily: 'var(--font-sans)',
            }}>
              <window.Icon name="generate" size={26} color="var(--cg-text-muted)" />
            ...
            {list.map((t) => <TemplateCard key={t.id} tpl={t} onOpen={onOpen} />)}
          </div>
        </main>
      </div>
    </div>
