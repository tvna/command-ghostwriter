...
const { Toggle, TextInput, Selectbox, RadioGroup, Button, Divider } =
...
function Sub({ children }) {
  return (
    <h4 style={{ margin: '0 0 2px', fontFamily: 'var(--font-sans)', fontSize: 'var(--text-sm)',
                 fontWeight: 700, color: 'var(--cg-text)' }}>{children}</h4>
  );
}
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        width: 620, maxHeight: '86vh', overflow: 'auto', background: 'var(--cg-bg)',
        border: '1px solid var(--cg-border)', borderRadius: 'var(--radius-lg)',
        boxShadow: 'var(--shadow-lg)', fontFamily: 'var(--font-sans)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                      padding: '16px 20px', borderBottom: '1px solid var(--cg-border)' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 'var(--text-lg)', fontWeight: 700, color: 'var(--cg-text)', whiteSpace: 'nowrap' }}>
            <window.Icon name="settings" size={18} />詳細設定
          </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)',
                      padding: '14px 20px', borderTop: '1px solid var(--cg-border)' }}>
